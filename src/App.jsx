...
const GEO = {
  // EUROPA
  carcassonne:     {x:450, y:175, label:"Carcassonne"},
  chateau_gaillard:{x:448, y:160, label:"Ch. Gaillard"},
  coucy:           {x:455, y:158, label:"Coucy"},
  mont_michel:     {x:443, y:162, label:"Mont S-Michel"},
  harlech:         {x:438, y:152, label:"Harlech"},
  caernarvon:      {x:436, y:150, label:"Caernarvon"},
  windsor:         {x:442, y:148, label:"Windsor"},
  bodiam:          {x:444, y:149, label:"Bodiam"},
  alhambra:        {x:438, y:185, label:"Alhambra"},
  // NAHER OSTEN & MITTELMEER
  constantinople:  {x:543, y:168, label:"Konstantinopel"},
  rhodes:          {x:540, y:180, label:"Rhodos"},
  krak:            {x:562, y:185, label:"Krak"},
  akkon:           {x:559, y:188, label:"Akkon"},
  masada:          {x:561, y:192, label:"Masada"},
  babylon:         {x:576, y:186, label:"Babylon"},
  persepolis:      {x:590, y:190, label:"Persepolis"},
  alamut:          {x:585, y:173, label:"Alamut"},
  // ASIEN
  mehrangarh:      {x:623, y:195, label:"Mehrangarh"},
  great_wall:      {x:720, y:158, label:"Große Mauer"},
  himeji:          {x:770, y:170, label:"Himeji"},
  angkor:          {x:720, y:228, label:"Angkor"},
};

// Mittelerde map positions (fictional — on a 600x400 canvas)
const ME_GEO = {
  // Mittelerde
  angband:         {x:140, y: 55, label:"Angband"},
  gondolin:        {x:185, y: 72, label:"Gondolin"},
  erebor:          {x:300, y: 88, label:"Erebor"},
  khazad_dum:      {x:215, y:110, label:"Moria"},
  isengard:        {x:190, y:165, label:"Isengard"},
  edoras:          {x:210, y:175, label:"Edoras"},
  helmsdeep:       {x:195, y:180, label:"Helms Klamm"},
  minas_tirith:    {x:240, y:195, label:"Minas Tirith"},
  minas_morgul:    {x:256, y:200, label:"M. Morgul"},
  barad_dur:       {x:270, y:210, label:"Barad-dûr"},
  // Westeros
  winterfell:      {x:430, y: 75, label:"Winterfell"},
  harrenhal:       {x:445, y:145, label:"Harrenhal"},
  kings_landing:   {x:455, y:165, label:"Königsmund"},
};

function RealWorldMap({castles,onSelect,selected}){
  const [hov,setHov]=useState(null);
  const real=castles.filter(c=>c.type==="real"&&GEO[c.id]);
  const hovC=hov?castles.find(c=>c.id===hov):null;
  ...
  return(
    <div style={{position:"relative",background:"rgba(0,0,0,0.3)",borderRadius:"8px",
      ...
      <svg viewBox="0 0 1000 500" style={{width:"100%",display:"block"}}>
        <defs>
          <radialGradient id="rw_ocean" cx="50%" cy="50%" r="70%">
        ...
        {/* Ocean */}
        <rect width="1000" height="500" fill="url(#rw_ocean)"/>
        {/* Latitude grid lines */}
        {[100,167,233,300,367,433].map(y=>(
          <line key={y} x1="0" y1={y} x2="1000" y2={y} stroke="rgba(100,160,255,0.06)" strokeWidth="0.5"/>
        ))}
        {/* Longitude grid lines */}
        {[100,200,300,400,500,600,700,800,900].map(x=>(
          <line key={x} x1={x} y1="0" x2={x} y2="500" stroke="rgba(100,160,255,0.04)" strokeWidth="0.5"/>
        ))}

        {/* ── CONTINENTS ── */}
        {/* North America */}
        <path d="M80,60 L130,50 L170,55 L195,75 L200,100 L190,130 L175,160 L155,185
                 L140,200 L120,210 L100,220 L80,230 L65,215 L55,195 L50,170
                 L55,140 L60,110 L65,80 Z"
          fill="rgba(55,80,38,0.55)" stroke="rgba(70,100,48,0.5)" strokeWidth="1"/>
        {/* Greenland */}
        <path d="M280,30 L320,25 L350,35 L345,55 L320,60 L285,52 Z"
          fill="rgba(55,80,38,0.4)" stroke="rgba(70,100,48,0.4)" strokeWidth="0.8"/>
        {/* South America */}
        <path d="M185,255 L210,245 L230,250 L240,270 L245,300 L240,340
                 L225,370 L205,390 L188,380 L178,355 L175,320 L178,285 Z"
          fill="rgba(70,90,32,0.5)" stroke="rgba(85,108,40,0.45)" strokeWidth="1"/>
        {/* Europe */}
        <path d="M400,80 L420,70 L445,68 L470,72 L490,80 L500,95 L495,110
                 L480,120 L465,130 L450,135 L435,130 L420,120 L408,108 L400,95 Z"
          fill="rgba(55,80,38,0.6)" stroke="rgba(70,100,48,0.55)" strokeWidth="1"/>
        {/* Iberia */}
        <path d="M415,130 L435,128 L445,138 L442,155 L428,160 L414,152 L410,138 Z"
          fill="rgba(55,80,38,0.55)" stroke="rgba(70,100,48,0.5)" strokeWidth="0.8"/>
        {/* Italy */}
        <path d="M460,120 L468,128 L472,145 L470,162 L465,170 L458,162 L455,145 L454,128 Z"
          fill="rgba(55,80,38,0.5)" stroke="rgba(70,100,48,0.45)" strokeWidth="0.7"/>
        {/* Scandinavia */}
        <path d="M455,60 L468,52 L478,55 L482,68 L475,80 L465,82 L455,75 Z"
          fill="rgba(55,80,38,0.5)" stroke="rgba(70,100,48,0.45)" strokeWidth="0.7"/>
        {/* British Isles */}
        <ellipse cx="430" cy="95" rx="10" ry="18" fill="rgba(55,80,38,0.55)" stroke="rgba(70,100,48,0.5)" strokeWidth="0.7"/>
        <ellipse cx="420" cy="90" rx="6" ry="12" fill="rgba(55,80,38,0.5)" stroke="rgba(70,100,48,0.45)" strokeWidth="0.6"/>
        {/* Africa */}
        <path d="M420,165 L445,158 L470,160 L495,168 L515,180 L525,205 L528,240
                 L522,275 L508,308 L490,335 L468,352 L445,358 L422,350 L405,330
                 L395,302 L392,268 L395,235 L402,205 L410,182 Z"
          fill="rgba(90,70,28,0.55)" stroke="rgba(110,85,35,0.5)" strokeWidth="1"/>
        {/* Mediterranean */}
        <path d="M415,155 L500,152 L530,158 L545,165 L540,172 L510,175
                 L480,172 L455,168 L430,168 L415,162 Z"
          fill="rgba(13,30,56,0.9)" stroke="none"/>
        {/* Middle East */}
        <path d="M528,158 L570,155 L595,160 L605,175 L600,195 L585,210
                 L565,215 L545,205 L530,188 L525,172 Z"
          fill="rgba(90,70,28,0.52)" stroke="rgba(110,85,35,0.45)" strokeWidth="0.8"/>
        {/* Arabia */}
        <path d="M545,188 L580,183 L605,190 L612,215 L605,245 L588,260
                 L565,262 L548,248 L540,225 L540,205 Z"
          fill="rgba(110,85,32,0.48)" stroke="rgba(130,100,38,0.4)" strokeWidth="0.8"/>
        {/* Asia main */}
        <path d="M510,65 L560,58 L620,55 L680,57 L740,60 L790,65 L830,72
                 L855,85 L860,105 L845,125 L815,140 L775,148 L730,150 L685,148
                 L645,145 L605,148 L570,152 L545,150 L520,140 L508,120 L505,98 Z"
          fill="rgba(55,80,38,0.58)" stroke="rgba(70,100,48,0.52)" strokeWidth="1"/>
        {/* India */}
        <path d="M605,148 L635,145 L648,155 L650,178 L642,202 L625,218
                 L608,220 L595,208 L588,185 L590,162 Z"
          fill="rgba(55,80,38,0.52)" stroke="rgba(70,100,48,0.45)" strokeWidth="0.8"/>
        {/* SE Asia */}
        <path d="M720,155 L750,152 L768,160 L772,178 L760,190 L740,192
                 L722,182 L715,168 Z"
          fill="rgba(55,80,38,0.5)" stroke="rgba(70,100,48,0.43)" strokeWidth="0.7"/>
        {/* Japan */}
        <ellipse cx="790" cy="138" rx="8" ry="22" fill="rgba(55,80,38,0.55)"
          stroke="rgba(70,100,48,0.5)" strokeWidth="0.7" transform="rotate(-20 790 138)"/>
        {/* Australia */}
        <path d="M730,310 L780,300 L820,305 L845,320 L850,345 L840,370
                 L815,385 L782,388 L752,378 L732,358 L722,335 Z"
          fill="rgba(90,70,28,0.45)" stroke="rgba(110,85,35,0.4)" strokeWidth="0.8"/>

        {/* ── CASTLE PINS ── */}
        <g filter="url(#rw_glow)">
          {real.map(c=>{
            const g=GEO[c.id];
            ...
            return(
              <g key={c.id} style={{cursor:"pointer"}}
                ...
                {/* Glow */}
                {(iS||isH)&&<circle cx={g.x} cy={g.y} r={iS?10:7}
                  fill={ac} opacity="0.15"/>}
                {/* Pin dot */}
                <circle cx={g.x} cy={g.y} r={iS?5:isH?4:3}
                  fill={iS?ac:`${ac}cc`}
                  stroke={iS?"white":ac}
                  strokeWidth={iS?1.2:0.6}
                  style={{transition:"all .15s"}}/>
                {/* Label */}
                {(iS||isH)&&<>
                  <rect x={g.x+6} y={g.y-7} width={g.label.length*4.5} height="13" rx="2"
                    fill="rgba(5,3,1,0.92)" stroke={ac} strokeWidth="0.5"/>
                  <text x={g.x+8} y={g.y+3}
                    fill={ac} fontSize="9" fontFamily="'Palatino Linotype',Georgia,serif">
                    {g.label}
                  </text>
                </>}
              </g>
        ...
        {/* Compass */}
        <g transform="translate(960,40)" opacity="0.5">
          <circle cx="0" cy="0" r="14" fill="rgba(0,0,0,0.4)" stroke="rgba(201,168,76,0.3)" strokeWidth="0.8"/>
          <text x="0" y="-5" textAnchor="middle" fill="rgba(201,168,76,0.8)" fontSize="10" fontFamily="serif" fontWeight="bold">N</text>
          <line x1="0" y1="-2" x2="0" y2="10" stroke="rgba(201,168,76,0.4)" strokeWidth="0.8"/>
        </g>
      </svg>
    </div>
  );
  ...
  return(
    <div style={{position:"relative",background:"rgba(0,0,0,0.3)",borderRadius:"8px",
      ...
      <svg viewBox="0 0 600 400" style={{width:"100%",display:"block"}}>
        <defs>
          <radialGradient id="me_bg" cx="45%" cy="50%" r="60%">
        ...
        {/* ── MITTELERDE ── */}
        {/* Divider line between ME and Westeros */}
        <line x1="370" y1="0" x2="370" y2="400" stroke="rgba(100,60,180,0.15)" strokeWidth="1" strokeDasharray="6,4"/>
        {/* Section labels */}
        <text x="185" y="22" textAnchor="middle" fill="rgba(150,100,220,0.35)" fontSize="13"
          fontFamily="'Palatino Linotype',Georgia,serif" letterSpacing="2">✦ MITTELERDE</text>
        <text x="490" y="22" textAnchor="middle" fill="rgba(100,140,200,0.35)" fontSize="13"
          fontFamily="'Palatino Linotype',Georgia,serif" letterSpacing="2">❄ WESTEROS</text>

        {/* Mittelerde terrain */}
        {/* Thangorodrim mountains (north) */}
        {[100,140,185,230].map((x,i)=>(
          <polygon key={i} points={`${x},${50-i*5} ${x-18},${85} ${x+18},${85}`}
            fill="rgba(55,42,65,0.6)" stroke="rgba(75,55,90,0.4)" strokeWidth="0.8"/>
        ))}
        {/* Misty Mountains */}
        {[180,205,225,245,262].map((x,i)=>(
          <polygon key={i} points={`${x},${100+i*8} ${x-12},${145+i*5} ${x+12},${145+i*5}`}
            fill="rgba(45,38,58,0.55)" stroke="rgba(62,50,78,0.38)" strokeWidth="0.7"/>
        ))}
        {/* Ered Nimrais */}
        {[185,210,238,262,288].map((x,i)=>(
          <polygon key={i} points={`${x},${215-i*3} ${x-10},${248} ${x+10},${248}`}
            fill="rgba(45,38,58,0.5)" stroke="rgba(62,50,78,0.35)" strokeWidth="0.6"/>
        ))}
        {/* Mordor (dark) */}
        <path d="M230,215 L300,210 L330,220 L340,245 L330,265 L295,272 L255,268 L230,255 L220,235 Z"
          fill="rgba(40,15,8,0.7)" stroke="rgba(60,20,10,0.5)" strokeWidth="1"/>
        <text x="278" y="245" textAnchor="middle" fill="rgba(140,40,20,0.35)" fontSize="10" fontFamily="serif">MORDOR</text>
        {/* Rohan plains */}
        <path d="M165,215 L230,210 L235,250 L225,270 L185,275 L155,265 L148,242 Z"
          fill="rgba(55,65,30,0.3)" stroke="rgba(70,85,38,0.25)" strokeWidth="0.7"/>
        <text x="192" y="250" textAnchor="middle" fill="rgba(100,120,50,0.3)" fontSize="9" fontFamily="serif">ROHAN</text>
        {/* Gondor */}
        <path d="M230,255 L285,250 L300,265 L295,290 L270,300 L240,295 L225,278 Z"
          fill="rgba(50,55,40,0.3)" stroke="rgba(65,72,50,0.25)" strokeWidth="0.7"/>
        <text x="263" y="280" textAnchor="middle" fill="rgba(90,100,65,0.3)" fontSize="9" fontFamily="serif">GONDOR</text>
        {/* The Shire */}
        <ellipse cx="135" cy="168" rx="25" ry="18" fill="rgba(55,75,30,0.3)" stroke="rgba(70,95,38,0.25)" strokeWidth="0.7"/>
        <text x="135" y="172" textAnchor="middle" fill="rgba(80,120,40,0.3)" fontSize="8" fontFamily="serif">Auenland</text>
        {/* Sea (west of ME) */}
        <path d="M0,0 L110,0 L100,400 L0,400 Z" fill="rgba(10,20,50,0.4)"/>
        <text x="55" y="200" textAnchor="middle" fill="rgba(50,90,160,0.25)" fontSize="11"
          fontFamily="serif" transform="rotate(-90 55 200)">BELEGAR</text>

        {/* ── WESTEROS ── */}
        {/* Main continent */}
        <path d="M385,40 L420,35 L455,38 L480,50 L492,75 L490,110 L482,150
                 L470,185 L460,220 L452,260 L448,300 L440,335 L428,360
                 L415,365 L400,355 L390,325 L385,290 L382,250 L383,210
                 L382,170 L380,130 L382,90 L384,58 Z"
          fill="rgba(42,55,35,0.5)" stroke="rgba(55,72,45,0.4)" strokeWidth="1"/>
        {/* The Wall (north) */}
        <rect x="383" y="58" width="95" height="5" rx="1"
          fill="rgba(200,220,240,0.4)" stroke="rgba(180,210,235,0.6)" strokeWidth="1"/>
        <text x="432" y="52" textAnchor="middle" fill="rgba(180,210,235,0.4)" fontSize="9" fontFamily="serif">THE WALL</text>
        {/* The North label */}
        <text x="435" y="90" textAnchor="middle" fill="rgba(80,100,140,0.3)" fontSize="9" fontFamily="serif">THE NORTH</text>
        {/* Narrow Sea (east) */}
        <path d="M490,60 L540,55 L560,80 L555,130 L545,175 L535,220
                 L525,260 L515,295 L505,315 L495,310 L490,280 L488,240
                 L490,195 L492,150 L492,100 Z"
          fill="rgba(10,25,55,0.5)"/>
        <text x="522" y="185" textAnchor="middle" fill="rgba(50,90,160,0.25)" fontSize="10"
          fontFamily="serif" transform="rotate(-90 522 185)">SCHMAL. SEE</text>
        ...
        <g filter="url(#me_glow)">
          {fantasy.map(c=>{
            ...
            const isH=hov===c.id;
            const ac=c.theme.accent;
            const isWesteros=["winterfell","harrenhal","kings_landing"].includes(c.id);
            return(
              <g key={c.id} style={{cursor:"pointer"}}
                onClick={()=>onSelect(c)}
                onMouseEnter={()=>setHov(c.id)}
                onMouseLeave={()=>setHov(null)}>
                {(iS||isH)&&<circle cx={g.x} cy={g.y} r={iS?9:6} fill={ac} opacity="0.15"/>}
                {/* Triangle for fantasy */}
                <polygon
                  points={`${g.x},${g.y-(iS?5:isH?4:3)} ${g.x+(iS?4:isH?3.5:2.5)},${g.y+(iS?3:isH?2.5:1.8)} ${g.x-(iS?4:isH?3.5:2.5)},${g.y+(iS?3:isH?2.5:1.8)}`}
                  fill={iS?ac:`${ac}cc`}
                  stroke={iS?"white":ac}
                  strokeWidth={iS?1:0.5}
                  style={{transition:"all .15s"}}/>
                {(iS||isH)&&<>
                  <rect x={g.x+5} y={g.y-7} width={g.label.length*4.2} height="13" rx="2"
                    fill="rgba(3,2,8,0.94)" stroke={ac} strokeWidth="0.5"/>
                  <text x={g.x+7} y={g.y+3}
                    fill={ac} fontSize="8.5" fontFamily="'Palatino Linotype',Georgia,serif">
                    {g.label}
                  </text>
                </>}
              </g>
    ...
    <div style={{padding:"18px 20px"}}>
      ...
      <div style={{display:"flex",gap:"18px",marginTop:"10px",padding:"8px 14px",
        ...
        <div style={{marginLeft:"auto",fontSize:"11px",color:"#5a4a28"}}>
          {castles.filter(c=>GEO[c.id]||ME_GEO[c.id]).length} Festungen kartiert
        </div>
      </div>
    </div>
  );
