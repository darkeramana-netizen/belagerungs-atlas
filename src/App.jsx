...
// ── WorldMap ───────────────────────────────────────────────────────────────
// Regional tile map — guaranteed correct pin placement, no SVG continent issues
const REGION_MAP_DATA = {
  europa: {
    label:"⚜ Europa", color:"#3a5a28", bg:"rgba(40,70,25,0.12)", border:"rgba(60,100,35,0.25)",
    castles:["carcassonne","chateau_gaillard","coucy","mont_michel","harlech","caernarvon","windsor","bodiam","alhambra","krak","constantinople","rhodes","akkon"],
  },
  nahost: {
    label:"☪ Naher Osten", color:"#7a5a20", bg:"rgba(100,75,25,0.12)", border:"rgba(140,100,35,0.25)",
    castles:["masada","babylon","persepolis","alamut","mehrangarh"],
  },
  ostasien: {
    label:"⛩ Ostasien", color:"#3a6a4a", bg:"rgba(35,80,50,0.12)", border:"rgba(50,110,65,0.25)",
    castles:["himeji","great_wall","angkor"],
  },
  mittelerde: {
    label:"✦ Mittelerde", color:"#6a50a0", bg:"rgba(60,35,110,0.12)", border:"rgba(90,55,160,0.25)",
    castles:["gondolin","erebor","angband","khazad_dum","isengard","edoras","helmsdeep","minas_tirith","minas_morgul","barad_dur"],
  },
  westeros: {
    label:"❄ Westeros", color:"#4a6888", bg:"rgba(40,65,100,0.12)", border:"rgba(55,90,140,0.25)",
    castles:["winterfell","harrenhal","kings_landing"],
  },
};
  ...
  return(
    <div style={{padding:"18px 20px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:"16px"}}>
        <div>
          <div style={{fontSize:"18px",fontWeight:"bold",color:"#f0e6cc",letterSpacing:"0.5px",marginBottom:"4px"}}>🌍 Weltkarte der Festungen</div>
          <div style={{fontSize:"12px",color:"#9a8a68"}}>● Historisch &nbsp;▲ Fantasy · Hover für Details · Klicken zum Analysieren</div>
        </div>
        <div style={{fontSize:"12px",color:"#9a8a68",textAlign:"right"}}>
          <div>{castles.filter(c=>c.type==="real").length} historische Burgen</div>
          <div>{castles.filter(c=>c.type==="fantasy").length} Fantasy-Festungen</div>
        </div>
      </div>

      {/* Regional tile grid */}
      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gridTemplateRows:"auto auto",gap:"8px"}}>

        {/* EUROPA — large tile spanning 2 rows */}
        <div style={{gridRow:"1/3",padding:"14px",background:REGION_MAP_DATA.europa.bg,
          border:`1px solid ${REGION_MAP_DATA.europa.border}`,borderRadius:"8px",position:"relative"}}>
          <div style={{fontSize:"11px",color:REGION_MAP_DATA.europa.color,letterSpacing:"2px",
            fontWeight:"bold",marginBottom:"12px"}}>{REGION_MAP_DATA.europa.label}</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"6px"}}>
            {REGION_MAP_DATA.europa.castles.map(id=>{
              const c=castles.find(x=>x.id===id);
              if(!c)return null;
              const iS=selected?.id===id,isH=hov===id;
              return(
                <button key={id} onClick={()=>onSelect(c)}
                  onMouseEnter={()=>setHov(id)} onMouseLeave={()=>setHov(null)}
                  style={{padding:"8px 4px",background:iS?`${c.theme.accent}22`:isH?`${c.theme.accent}11`:"rgba(255,255,255,0.03)",
                    border:`1px solid ${iS?c.theme.accent+"66":isH?c.theme.accent+"33":"rgba(255,255,255,0.06)"}`,
                    borderRadius:"6px",cursor:"pointer",transition:"all .15s",textAlign:"center"}}>
                  <div style={{fontSize:"18px",marginBottom:"3px"}}>{c.icon}</div>
                  <div style={{fontSize:"10px",color:iS?c.theme.accent:isH?"#d0c090":"#7a6a40",
                    lineHeight:1.2,fontWeight:iS?"bold":"normal"}}>
                    {c.name.split(" ").slice(0,2).join(" ")}
                  </div>
                  <div style={{fontSize:"9px",color:rCol(avg(c)),fontFamily:"monospace",marginTop:"2px"}}>{avg(c)}</div>
                </button>
              );
            })}
          </div>
        ...
        {/* NAHER OSTEN */}
        <div style={{padding:"14px",background:REGION_MAP_DATA.nahost.bg,
          border:`1px solid ${REGION_MAP_DATA.nahost.border}`,borderRadius:"8px"}}>
          <div style={{fontSize:"11px",color:REGION_MAP_DATA.nahost.color,letterSpacing:"2px",
            fontWeight:"bold",marginBottom:"10px"}}>{REGION_MAP_DATA.nahost.label}</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"5px"}}>
            {REGION_MAP_DATA.nahost.castles.map(id=>{
              const c=castles.find(x=>x.id===id);
              if(!c)return null;
              const iS=selected?.id===id,isH=hov===id;
              return(
                <button key={id} onClick={()=>onSelect(c)}
                  onMouseEnter={()=>setHov(id)} onMouseLeave={()=>setHov(null)}
                  style={{padding:"7px 4px",background:iS?`${c.theme.accent}22`:isH?`${c.theme.accent}11`:"rgba(255,255,255,0.03)",
                    border:`1px solid ${iS?c.theme.accent+"66":isH?c.theme.accent+"33":"rgba(255,255,255,0.06)"}`,
                    borderRadius:"6px",cursor:"pointer",transition:"all .15s",textAlign:"center"}}>
                  <div style={{fontSize:"17px",marginBottom:"2px"}}>{c.icon}</div>
                  <div style={{fontSize:"9px",color:iS?c.theme.accent:isH?"#d0c090":"#7a6a40",lineHeight:1.2}}>
                    {c.name.split(" ")[0]}
                  </div>
                  <div style={{fontSize:"8px",color:rCol(avg(c)),fontFamily:"monospace",marginTop:"1px"}}>{avg(c)}</div>
                </button>
              );
            })}
          </div>
        ...
        {/* OSTASIEN */}
        <div style={{padding:"14px",background:REGION_MAP_DATA.ostasien.bg,
          border:`1px solid ${REGION_MAP_DATA.ostasien.border}`,borderRadius:"8px"}}>
          <div style={{fontSize:"11px",color:REGION_MAP_DATA.ostasien.color,letterSpacing:"2px",
            fontWeight:"bold",marginBottom:"10px"}}>{REGION_MAP_DATA.ostasien.label}</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"5px"}}>
            {REGION_MAP_DATA.ostasien.castles.map(id=>{
              const c=castles.find(x=>x.id===id);
              if(!c)return null;
              const iS=selected?.id===id,isH=hov===id;
              return(
        ...
        {/* MITTELERDE */}
        <div style={{padding:"14px",background:REGION_MAP_DATA.mittelerde.bg,
          border:`1px solid ${REGION_MAP_DATA.mittelerde.border}`,borderRadius:"8px"}}>
          <div style={{fontSize:"11px",color:REGION_MAP_DATA.mittelerde.color,letterSpacing:"2px",
            fontWeight:"bold",marginBottom:"10px"}}>{REGION_MAP_DATA.mittelerde.label}</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"5px"}}>
            {REGION_MAP_DATA.mittelerde.castles.map(id=>{
              const c=castles.find(x=>x.id===id);
              if(!c)return null;
              const iS=selected?.id===id,isH=hov===id;
              return(
        ...
        {/* WESTEROS */}
        <div style={{padding:"14px",background:REGION_MAP_DATA.westeros.bg,
          border:`1px solid ${REGION_MAP_DATA.westeros.border}`,borderRadius:"8px"}}>
          <div style={{fontSize:"11px",color:REGION_MAP_DATA.westeros.color,letterSpacing:"2px",
            fontWeight:"bold",marginBottom:"10px"}}>{REGION_MAP_DATA.westeros.label}</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"5px"}}>
            {REGION_MAP_DATA.westeros.castles.map(id=>{
              const c=castles.find(x=>x.id===id);
              if(!c)return null;
              const iS=selected?.id===id,isH=hov===id;
              return(
      ...
      {hovC&&(
        <div style={{marginTop:"10px",padding:"12px 16px",
          background:`linear-gradient(135deg,${hovC.theme.bg} 0%,rgba(12,9,4,0.98) 100%)`,
          border:`1px solid ${hovC.theme.accent}44`,borderLeft:`4px solid ${hovC.theme.accent}`,
          borderRadius:"6px",display:"flex",gap:"14px",alignItems:"center",
          boxShadow:`0 4px 20px rgba(0,0,0,0.5), 0 0 12px ${hovC.theme.accent}14`}}>
          <div style={{fontSize:"28px"}}>{hovC.icon}</div>
          <div style={{flex:1}}>
            <div style={{fontSize:"14px",fontWeight:"bold",color:"#f0e8d0",marginBottom:"2px"}}>{hovC.name}</div>
            <div style={{fontSize:"11px",color:hovC.theme.accent,marginBottom:"2px"}}>{hovC.sub}</div>
            <div style={{fontSize:"10px",color:"#7a6a48"}}>{hovC.loc} · {hovC.era}</div>
          </div>
          <div style={{textAlign:"center",flexShrink:0,padding:"8px 14px",
            background:"rgba(0,0,0,0.3)",borderRadius:"4px",border:`1px solid ${hovC.theme.accent}22`}}>
            <div style={{fontSize:"22px",fontWeight:"bold",color:rCol(avg(hovC))}}>{avg(hovC)}</div>
            <div style={{fontSize:"9px",color:"#5a4a28",letterSpacing:"1px",marginTop:"1px"}}>SCORE</div>
          </div>
          <button onClick={()=>onSelect(hovC)}
            style={{padding:"8px 14px",background:`${hovC.theme.accent}18`,
              border:`1px solid ${hovC.theme.accent}44`,borderRadius:"4px",
              color:hovC.theme.accent,cursor:"pointer",fontSize:"11px",letterSpacing:"1px"}}>
            ÖFFNEN →
          </button>
        </div>
      )}
