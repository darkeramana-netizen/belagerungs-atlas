  ...
  const MapPin=({c,pos,shape="circle"})=>{
    ...
    return(
      <g onClick={()=>onSelect(c)} onMouseEnter={e=>handleEnter(c,e)} onMouseLeave={handleLeave} style={{cursor:"pointer"}}>
        ...
        {(iS||iH)&&<circle cx={pos.x} cy={pos.y} r={iS?4:3}
          fill="none" stroke={ac} strokeWidth="0.4" strokeOpacity={iS?"0.5":"0.3"}
          style={{animation:"pulse 2s ease infinite"}}/>}
        {/* Outer glow blob */}
        <circle cx={pos.x} cy={pos.y} r={iS?2.5:iH?2:1.5}
          fill={ac} fillOpacity={iS?0.15:iH?0.1:0.05}/>
        {shape==="circle"
          ? <circle cx={pos.x} cy={pos.y} r={iS?2.2:iH?1.8:1.3}
              fill={iS?ac:`${ac}dd`} fillOpacity={iS?1:iH?0.9:0.7}
              stroke={iS?"rgba(255,255,255,0.9)":ac} strokeWidth={iS?0.6:0.3}
              style={{transition:"all .2s ease"}}/>
          : <polygon
              points={`${pos.x},${pos.y-(iS?2.6:iH?2.1:1.5)} ${pos.x+(iS?2.2:iH?1.8:1.3)},${pos.y+(iS?1.3:iH?1.0:0.8)} ${pos.x-(iS?2.2:iH?1.8:1.3)},${pos.y+(iS?1.3:iH?1.0:0.8)}`}
        ...
        {iS&&<>
          <rect x={pos.x-7} y={pos.y-7.5} width="14" height="4" rx="0.7"
            fill="rgba(6,4,2,0.95)" stroke={ac} strokeWidth="0.4" strokeOpacity="0.8"/>
          <text x={pos.x} y={pos.y-5.5} textAnchor="middle" dominantBaseline="middle"
            fill={ac} fontSize="7" fontFamily="'Palatino Linotype',Georgia,serif" letterSpacing="0.1">
        ...
        {iH&&!iS&&<>
          ...
          <text x={pos.x} y={pos.y-4.8} textAnchor="middle" dominantBaseline="middle"
            fill="rgba(240,220,180,0.9)" fontSize="7" fontFamily="'Palatino Linotype',Georgia,serif">
            {c.name.split(" ")[0].slice(0,10)}
          </text>
        </>}
      </g>
    ...
    <div style={{padding:"18px 20px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:"14px"}}>
        <div>
          ...
          <div style={{fontSize:"13px",color:"#b09a70",letterSpacing:"1px"}}>● HISTORISCH  ▲ FANTASY-REICH  · Hover für Details · Klicken zum Analysieren</div>
        </div>
        <div style={{textAlign:"right",fontSize:"13px",color:"#b09a70"}}>
      ...
      <div style={{position:"relative",borderRadius:"8px",overflow:"hidden",
        ...
        <svg ref={svgRef} viewBox="0 0 100 62" style={{width:"100%",display:"block"}}>
          ...
          <text x="19" y="4" textAnchor="middle" fill="rgba(150,100,220,0.45)" fontSize="7"
            fontFamily="'Palatino Linotype',Georgia,serif" letterSpacing="0.3">✦ MITTELERDE</text>
          <text x="8" y="28.5" textAnchor="middle" fill="rgba(100,130,200,0.4)" fontSize="7"
        ...
        {hC&&<div style={{position:"absolute",bottom:"10px",left:"12px",right:"12px",
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:"13px",color:"#cbb888",letterSpacing:"0.5px"}}>{hC.sub}</div>
            <div style={{fontSize:"12px",color:"#b09a70",marginTop:"2px"}}>{hC.loc} · {hC.era}</div>
          </div>
          <div style={{textAlign:"center",flexShrink:0}}>
            <div style={{fontSize:"20px",fontWeight:"bold",color:rCol(avg(hC)),lineHeight:1}}>{avg(hC)}</div>
            <div style={{fontSize:"11px",color:"#9a8a68",letterSpacing:"1px",marginTop:"1px"}}>SCORE</div>
          </div>
          <div style={{fontSize:"12px",color:hC.theme.accent,flexShrink:0,letterSpacing:"1px"}}>KLICKEN →</div>
        </div>}
      </div>
      ...
      <div style={{display:"flex",gap:"18px",marginTop:"10px",padding:"8px 12px",background:"rgba(0,0,0,0.25)",borderRadius:"4px",border:"1px solid rgba(255,255,255,0.04)"}}>
        <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
          ...
          <span style={{fontSize:"13px",color:"#4a3a20"}}>Historische Burg</span>
        </div>
        <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
          <svg width="12" height="12" viewBox="0 0 12 12"><polygon points="6,1 11,10 1,10" fill="rgba(136,102,204,0.7)"/></svg>
          <span style={{fontSize:"13px",color:"#4a3a20"}}>Fantasy-Festung</span>
        </div>
        <div style={{display:"flex",gap:"6px",alignItems:"center"}}>
          ...
          <span style={{fontSize:"13px",color:"#4a3a20"}}>Ausgewählt</span>
        </div>
        <div style={{marginLeft:"auto",fontSize:"13px",color:"#9a8a68"}}>{real.length + fantasy.length} Festungen kartiert</div>
      </div>
    </div>
  );
}
...
function WhatIf({castle}){
  ...
  const analyze=async()=>{
    ...
    try{
      ...
      if(!apiData){
        // Generate scenario-specific fallback based on the scenario text
        const s=scen.toLowerCase();
        const fell=castle.ratings.walls<85||s.includes("rampe")||s.includes("drach")||s.includes("verräter")||s.includes("tunnel");
        const plaus=s.includes("doppelt")||s.includes("mehr")?8:s.includes("modern")||s.includes("kanone")?9:s.includes("verräter")?7:5;
        const analyses=[
          `Diese Änderung hätte die Verteidigungsfähigkeit von ${castle.name} grundlegend verändert. ${castle.verdict} Mit diesem Szenario wäre der historische Ausgang möglicherweise ein völlig anderer gewesen.`,
          `${castle.name} war bereits ohne diese Veränderung eine beeindruckende Festung. Die wichtigste Schwäche war ${castle.weaknesses[0].toLowerCase()} — das Szenario adressiert genau diesen Punkt.`,
          `Kontrafaktisch betrachtet: ${castle.history} Ein anderer Verlauf wäre plausibel, aber Burgen fallen meist durch mehrere Faktoren gleichzeitig.`,
        ];
        setRes({
          likelihood:plaus,
          outcome:`${castle.name} hätte unter diesen Umständen ${fell?"wahrscheinlich länger gehalten":"möglicherweise schneller kapituliert"}.`,
          analysis:analyses[Math.floor(Math.random()*analyses.length)],
          wouldHaveFallen:!fell,
          keyFactor:fell?`${castle.strengths[0]} wäre der entscheidende Vorteil gewesen.`:`${castle.weaknesses[0]} bleibt die kritische Achillesferse.`,
          timeChange:fell?`Die Burg hätte ${Math.floor(Math.random()*50+20)} Jahre länger gehalten.`:`Der Fall wäre ${Math.floor(Math.random()*5+2)} Jahre früher eingetreten.`,
          historicalParallels:[
            `${castle.name} (${castle.era}) — ähnliche Szenarien in der Geschichte`,
            `Andere ${castle.epoch}-Burgen mit vergleichbaren Veränderungen`,
          ],
          confidence:plaus>7?"hoch":"mittel",
        });
      }
