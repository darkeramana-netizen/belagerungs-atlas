                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"14px"}}>
                        ...
                        <div style={{display:"flex",flexDirection:"column",gap:"8px"}}>
                          ...
                          <div style={{padding:"12px 14px",background:"rgba(0,0,0,0.2)",
                            ...
                            <div style={{fontSize:"13px",color:"#7a6a50",lineHeight:1.85}}>{sel.desc}</div>
                          </div>
                          ...
                          {GEO[sel.id]?(()=>{
                            ...
                            return(
                              <div style={{borderRadius:"8px",overflow:"hidden",
                                ...
                                <div id={`mini-map-${sel.id}`} style={{height:"320px",width:"100%"}}
                                  ref={el=>{
                                    ...
                                    const icon=L.divIcon({
                                      html:`<div style="width:18px;height:18px;background:${sel.theme.accent};border-radius:50%;border:2.5px solid white;box-shadow:0 0 12px ${sel.theme.accent};"></div>`,
                                      className:'',iconSize:[18,18],iconAnchor:[9,9]
                                    });
                                    L.marker([lat,lon],{icon}).addTo(map)
                                      .bindPopup(`<div style="background:rgba(10,7,3,0.97);border:1px solid ${sel.theme.accent};color:${sel.theme.accent};padding:6px 10px;border-radius:4px;font-family:Georgia,serif;font-size:13px"><strong>${sel.icon} ${sel.name}</strong><br><span style="color:#8a7a58;font-size:11px">${sel.loc}</span></div>`,{className:'custom-tooltip',opacity:1})
                                      .openPopup();
                                  }}/>
                              </div>
                            );
...
function CastleGrid({castles,onSelect,scores,filter,setFilter,epochFilter,setEpochFilter,regionFilter,setRegionFilter,search,setSearch}){
  ...
  const CastleCard=({c})=>{
    ...
    const cats=[{k:"walls",l:"Mauern",i:"🧱"},{k:"position",l:"Position",i:"⛰️"},{k:"supply",l:"Versorg.",i:"🍖"},{k:"garrison",l:"Garnison",i:"⚔️"},{k:"morale",l:"Moral",i:"🔥"}];
    const maxR=Math.max(...Object.values(c.ratings));
    const minR=Math.min(...Object.values(c.ratings));
    const verdictShort=c.verdict?c.verdict.slice(0,70)+"…":"";
    return(
      <button onClick={()=>onSelect(c)} onMouseEnter={()=>setHov(c.id)} onMouseLeave={()=>setHov(null)}
        ...
        <div className="castle-card" style={{
          ...
          background:isH
            ?`linear-gradient(160deg,${c.theme.bg} 0%,rgba(8,6,3,0.99) 100%)`
            :"rgba(255,255,255,0.015)",
          border:`1px solid ${isH?c.theme.accent+"77":"rgba(255,255,255,0.055)"}`,
          borderRadius:"10px",transition:"all .2s ease",
          transform:isH?"translateY(-4px) scale(1.01)":"none",
          boxShadow:isH
            ?`0 12px 40px rgba(0,0,0,0.6), 0 0 0 1px ${c.theme.accent}18, inset 0 1px 0 rgba(255,255,255,0.04)`
            :"0 1px 4px rgba(0,0,0,0.3)",
        }}>
          {/* Accent top bar — thicker when hovered */}
          <div style={{height:isH?"3px":"2px",
            background:`linear-gradient(90deg,${c.theme.accent},${c.theme.accent}44,transparent)`,
            opacity:isH?1:0.35,transition:"all .2s"}}/>

          {/* Hero section — icon + score */}
          <div style={{padding:"14px 13px 0",display:"flex",gap:"10px",alignItems:"flex-start"}}>
            {/* Big icon */}
            <div style={{fontSize:"32px",lineHeight:1,
              filter:isH?`drop-shadow(0 3px 8px ${c.theme.accent}44)`:"none",
              transition:"filter .2s",flexShrink:0}}>{c.icon}</div>
            <div style={{flex:1,minWidth:0}}>
              {/* Name */}
              <div style={{fontSize:"14px",fontWeight:"bold",
                color:isH?"#f5edd8":"#c8b888",
                lineHeight:1.2,marginBottom:"2px",
                whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{c.name}</div>
              <div style={{fontSize:"11px",color:isH?c.theme.accent:"#5a4a28",marginBottom:"1px"}}>{c.sub}</div>
              <div style={{fontSize:"10px",color:"#3a2a18"}}>{c.loc}</div>
            </div>
            {/* Score ring */}
            <div style={{flexShrink:0,position:"relative",width:"40px",height:"40px"}}>
              <svg width="40" height="40" viewBox="0 0 40 40">
                ...
                <circle cx="20" cy="20" r="17" fill="none" stroke={rCol(sc)} strokeWidth="3"
                  strokeDasharray={`${sc*1.068} 107`} strokeLinecap="round"
                  transform="rotate(-90 20 20)" opacity="0.9"
                  style={{transition:"stroke-dasharray 0.6s ease"}}/>
              </svg>
              <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",
                fontSize:"13px",fontWeight:"bold",color:rCol(sc),fontFamily:"monospace"}}>{sc}</div>
            </div>
          </div>

          {/* Era badge */}
          <div style={{padding:"5px 13px 0",display:"flex",gap:"5px",alignItems:"center"}}>
            <span style={{fontSize:"10px",color:"#3a2a18"}}>{c.era}</span>
            <span style={{fontSize:"10px",color:"#2a1a10"}}>·</span>
            <span style={{fontSize:"10px",padding:"1px 5px",borderRadius:"2px",letterSpacing:"0.5px",
              background:c.type==="real"?"rgba(40,70,25,0.3)":"rgba(50,30,80,0.35)",
            ...
            {hs&&<span style={{marginLeft:"auto",fontSize:"13px"}}>{hs.won?"✅":"❌"}</span>}
          </div>

          {/* Verdict teaser — only on hover */}
          {isH&&verdictShort&&(
            <div style={{padding:"7px 13px 0",fontSize:"11px",color:"#6a5a38",
              fontStyle:"italic",lineHeight:1.6,animation:"fadeIn 0.15s ease"}}>
              "{verdictShort}"
            </div>
          )}

          {/* Stat bars */}
          <div style={{padding:"8px 13px 10px",display:"flex",flexDirection:"column",gap:"4px"}}>
            {cats.map(cat=>{
              const v=c.ratings[cat.k],isMax=v===maxR,isMin=v===minR;
              return(
                <div key={cat.k} style={{display:"flex",alignItems:"center",gap:"5px"}}>
                  <div style={{fontSize:"10px",width:"44px",flexShrink:0,
                    color:isMax?c.theme.accent:isMin?"#663322":"#5a4a30"}}>
                    {cat.i} {cat.l}
                  </div>
                  <div style={{flex:1,height:"3px",background:"rgba(255,255,255,0.04)",borderRadius:"2px",overflow:"hidden"}}>
                    <div style={{height:"100%",
                      width:`${v}%`,
                      background:isMax
                        ?`linear-gradient(90deg,${c.theme.accent},${c.theme.accent}88)`
                        :isMin?"#993322":rCol(v),
                      borderRadius:"2px",
                      opacity:isH?1:0.6,
                      transition:"width .5s ease, opacity .2s"}}/>
                  </div>
                  <div style={{fontSize:"11px",fontFamily:"monospace",width:"20px",textAlign:"right",flexShrink:0,
                    color:isMax?c.theme.accent:isMin?"#774422":rCol(v),
                    fontWeight:isMax||isMin?"bold":"normal"}}>{v}</div>
                </div>
              );
            })}
          </div>
          ...
          {/* Bottom strip */}
          <div style={{padding:"5px 13px",borderTop:`1px solid ${isH?c.theme.accent+"22":"rgba(255,255,255,0.04)"}`,
            background:isH?"rgba(0,0,0,0.3)":"rgba(0,0,0,0.15)",
            display:"flex",justifyContent:"space-between",alignItems:"center",
            transition:"all .2s"}}>
            <span style={{fontSize:"10px",color:isH?c.theme.accent+"aa":"#2a1a08",letterSpacing:"1.5px",
              fontFamily:"serif"}}>{c.epoch.toUpperCase()}</span>
            {hs?.rating
              ?<span style={{fontSize:"11px",color:rCol(hs.rating*10),fontFamily:"monospace",fontWeight:"bold"}}>
                {hs.rating}/10
              </span>
              :<span style={{fontSize:"10px",color:"#2a1a08"}}>→</span>
            }
          </div>
        </div>
      </button>
