...
function lonlatToXY(lon, lat) {
  return { x: Math.round((lon + 180) / 360 * 1000), y: Math.round((90 - lat) / 180 * 500) };
}

// Historical castle positions (real lat/lon)
const GEO = {
  // lon, lat
  carcassonne:     lonlatToXY(2.35,  43.21),
  chateau_gaillard:lonlatToXY(1.41,  49.24),
  coucy:           lonlatToXY(3.32,  49.52),
  mont_michel:     lonlatToXY(-1.51, 48.64),
  harlech:         lonlatToXY(-4.11, 52.86),
  caernarvon:      lonlatToXY(-4.28, 53.14),
  windsor:         lonlatToXY(-0.61, 51.48),
  bodiam:          lonlatToXY(0.54,  51.00),
  alhambra:        lonlatToXY(-3.59, 37.18),
  constantinople:  lonlatToXY(28.98, 41.01),
  rhodes:          lonlatToXY(28.22, 36.44),
  krak:            lonlatToXY(36.26, 34.76),
  akkon:           lonlatToXY(35.07, 32.92),
  masada:          lonlatToXY(35.35, 31.32),
  babylon:         lonlatToXY(44.42, 32.54),
  persepolis:      lonlatToXY(52.89, 29.93),
  alamut:          lonlatToXY(50.57, 36.45),
  mehrangarh:      lonlatToXY(73.02, 26.30),
  great_wall:      lonlatToXY(116.57,40.68),
  himeji:          lonlatToXY(134.69,34.84),
  angkor:          lonlatToXY(103.87,13.41),
};
...
const CONTINENT_PATHS = [
  ...
  // Asia (main landmass)
  {d:"M 516 68 L 568 60 L 625 56 L 688 58 L 745 62 L 798 68 L 840 76 L 868 90 L 875 112 L 860 132 L 828 148 L 785 155 L 738 155 L 695 152 L 652 150 L 612 152 L 578 155 L 550 152 L 525 142 L 512 122 L 510 100 Z", fill:"rgba(52,78,35,0.62)"},
  // India subcontinent
  ...
  // SE Asia / Indochina
  {d:"M 730 155 L 762 150 L 778 162 L 780 180 L 768 194 L 748 196 L 730 184 L 724 168 Z", fill:"rgba(52,78,35,0.55)"},
  // Japan (elongated island)
  {d:"M 798 105 L 808 108 L 812 125 L 808 145 L 798 152 L 790 145 L 788 128 L 792 110 Z", fill:"rgba(52,78,35,0.6)"},
  // Australia
    ...
    <div style={{position:"relative",background:"rgba(0,0,0,0.3)",borderRadius:"8px",
      ...
      <svg viewBox="0 0 1000 500" style={{width:"100%",display:"block"}}>
        ...
        {[83,167,250,333,417].map(y=>(
          <line key={y} x1="0" y1={y} x2="1000" y2={y} stroke="rgba(100,160,255,0.05)" strokeWidth="0.5"/>
        ))}
        {/* Longitude grid */}
        ...
        {CONTINENT_PATHS.map((p,i)=>(
          <path key={i} d={p.d} fill={p.fill} stroke="rgba(70,100,45,0.35)" strokeWidth="0.6"/>
        ))}

        {/* Region labels */}
        <text x="125" y="140" textAnchor="middle" fill="rgba(80,120,50,0.25)" fontSize="11" fontFamily="serif">NORDAMERIKA</text>
        <text x="215" y="310" textAnchor="middle" fill="rgba(80,120,50,0.25)" fontSize="10" fontFamily="serif">SÜDAMERIKA</text>
        <text x="482" y="100" textAnchor="middle" fill="rgba(80,120,50,0.22)" fontSize="10" fontFamily="serif">EUROPA</text>
        <text x="480" y="260" textAnchor="middle" fill="rgba(100,80,30,0.22)" fontSize="11" fontFamily="serif">AFRIKA</text>
        <text x="680" y="105" textAnchor="middle" fill="rgba(80,120,50,0.22)" fontSize="11" fontFamily="serif">ASIEN</text>
        <text x="800" y="355" textAnchor="middle" fill="rgba(100,80,30,0.22)" fontSize="10" fontFamily="serif">AUSTRALIEN</text>
          ...
          {real.map(c=>{
            ...
            return(
              <g key={c.id} style={{cursor:"pointer"}}
                ...
                {(iS||isH)&&<circle cx={g.x} cy={g.y} r={iS?12:8} fill={ac} opacity="0.18"/>}
                <circle cx={g.x} cy={g.y} r={iS?6:isH?5:4}
                  fill={iS?ac:`${ac}dd`}
                  stroke={iS?"rgba(255,255,255,0.9)":isH?"rgba(255,255,255,0.5)":"rgba(0,0,0,0.5)"}
                  strokeWidth={iS?1.5:0.8}
                  style={{transition:"all .15s"}}/>
                {(iS||isH)&&(
                  <g>
                    <rect x={g.x+8} y={g.y-9} width={g.label.length*5.2+8} height="16" rx="3"
                      fill="rgba(4,2,1,0.94)" stroke={ac} strokeWidth="0.8"/>
                    <text x={g.x+12} y={g.y+4}
                      fill={ac} fontSize="10" fontFamily="'Palatino Linotype',Georgia,serif" fontWeight="bold">
                      {g.label}
                    </text>
                  </g>
                )}
        ...
        {/* Compass rose */}
        <g transform="translate(962,38)" opacity="0.55">
          <circle cx="0" cy="0" r="16" fill="rgba(0,0,0,0.5)" stroke="rgba(201,168,76,0.35)" strokeWidth="0.8"/>
